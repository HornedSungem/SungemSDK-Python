...
net = hs.SketchGuess(zoom = True, verbose = 2)
if WEBCAM: video_capture = cv2.VideoCapture(0)

# Label paths
fcl2 = open(hs.GetDefaultMiscRelPath('class_list.txt'),'r')
fcl = open(hs.GetDefaultMiscRelPath('class_list_chn.txt'),'r')
...
try:
    while True:
        if WEBCAM: _, image = video_capture.read()
        else: image = net.device.GetImage(True) # Only get image
        # v = create_string_buffer(640*360*3)
        # image = np.frombuffer(v.raw, dtype=np.uint8).reshape(360,640,3)
        # Image processing
        sz = image.shape
        cx = int(sz[0]/2)
        cy = int(sz[1]/2)
